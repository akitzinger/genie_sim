...
namespace gsi
{
      ...
      : opts_(opts)
  {
    ...
    //
    // The `map -> odom` edge is owned by the navigation / SLAM stack when real
    // localization is running. In sim-only setups, fake_slam supplies an
    // identity edge below. It is static because the identity has no time-varying
    // state and a dynamic publication can leave RViz requesting just beyond the
    // newest sample while the simulator is publishing.
    static_tf_broadcaster_ = std::make_shared<tf2_ros::StaticTransformBroadcaster>(node_);
    {
      ...
      if (opts_.fake_slam)
      {
        ...
        tf_m2o.header.stamp = tf_w2m.header.stamp;
        tf_m2o.header.frame_id = opts_.map_frame;
        tf_m2o.child_frame_id = opts_.odom_frame;
        tf_m2o.transform.rotation.w = 1.0;
        static_tf_broadcaster_->sendTransform(tf_m2o);
      }
    }
  ...
  {
    (void)sim_time;
    if (n != joint_names_.size())
    {
      return;
    }
    if (!pub_joint_->trylock())
    {
      return;
    }
  ...
  {
    (void)sim_time;
    if (n_bodies != body_frames_.size())
    {
      return;
    }
    if (!pub_tf_render_->trylock())
    {
      return;
    }
  ...
  {
    if (!cam_pose_dirty_.load(std::memory_order_acquire))
    {
      return false;
    }
    std::lock_guard<std::mutex> lk(cam_pose_mu_);
    if (!cam_pose_dirty_.load(std::memory_order_relaxed))
    {
      return false;
    }
